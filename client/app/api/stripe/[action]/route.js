...
async function handleAction(request, { params }) {
  ...
  if (!handler) {
    return new Response(JSON.stringify({ error: `Stripe action '${action}' not found` }), {
      status: 404,
      headers: { "Content-Type": "application/json" },
    });
  }

  // Parse raw body for signature verification checks
  ...
  try {
    ...
    console.error(`Error in stripe handler [${action}]:`, err);
